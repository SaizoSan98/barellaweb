...
export function Contact({ settings }: ContactProps) {
  const email = settings.contact_email || "barella.gep@gmail.com";
  const phone = settings.contact_phone || "+36 30 173 88 66";
  const address = settings.contact_address || "Magyarország";
  const ctaTitle = settings.cta_title || "Valósítsuk meg elképzeléseit.";
  const ctaDescription = settings.cta_description || "Legyen szó tervezésről vagy kivitelezésről, mi készen állunk a kihívásokra.";
  const footerDescription = settings.footer_description || "Ipari precizitású épületgépészeti megoldások. Innovatív technológiák és megbízható szakértelem egy helyen.";
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="mb-16 flex flex-col md:flex-row items-end justify-between gap-8 border-b border-white/10 pb-12">
            <div className="max-w-3xl">
                <div className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter leading-none mb-6">
                    {ctaTitle.includes("elképzeléseit") ? (
                      <>Valósítsuk meg <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-teal-400">elképzeléseit.</span></>
                    ) : ctaTitle}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-12">
            ...
            <div className="lg:col-span-4 space-y-8">
                ...
                <p className="text-gray-400 text-base leading-relaxed max-w-sm">{footerDescription}</p>
                <div className="flex gap-3">
                    <SocialButton icon={Facebook} href={facebook} />
                    <SocialButton icon={Instagram} href={instagram} />
                    <SocialButton icon={Linkedin} href={linkedin} />
                </div>
            </div>
            ...
            <div className="lg:col-span-5 space-y-4">
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <ContactCard icon={Phone} label="Telefon" value={phone} href={`tel:${phone.replace(/\s/g, '')}`} />
                    <ContactCard icon={MapPin} label="Székhely" value={address} fullWidth />
                 </div>
            </div>
        </div>
        ...
        <div className="border-t border-white/10 pt-8 relative overflow-hidden">
             ...
             <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-gray-500 text-xs relative z-20 pb-8">
                ...
                <div className="flex gap-6">
                    <a href="/documents/adatvedelmi_tajekoztato.pdf" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">Adatvédelem</a>
                    <a href="/documents/suti_tajekoztato.pdf" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">Süti kezelés</a>
                    <a href="/documents/aszf.pdf" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">ÁSZF</a>
                </div>
             </div>
        </div>
      </div>
