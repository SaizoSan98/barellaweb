...
const stepIcons = [CheckCircle2, Wrench, Clock, ShieldCheck];

export async function Process() {
  noStore();
  const steps = await db.select().from(processSteps).orderBy(asc(processSteps.order));
            ...
            return (
              <div key={step.id} className="relative z-10 flex flex-col items-center text-center group">
                <div className="w-24 h-24 rounded-full bg-black border-2 border-white/10 flex items-center justify-center mb-6 group-hover:border-primary group-hover:scale-110 transition-all duration-300 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                  <Icon className="w-10 h-10 text-gray-400 group-hover:text-primary transition-colors duration-300" />
                </div>
